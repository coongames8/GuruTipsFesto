...
export default function Payments({ setUserData }) {
  ...
  const renderPaymentType = useCallback(() => {
    ...
    switch (paymentType) {
      ...
      case "mpesa":
        return <KoraPayments key={key} setUserData={setUserData} />//<>{getCurrencyCode() === "KES" ? <PaystackPaymentsV1 setUserData={setUserData} /> : <KoraPayments key={key} setUserData={setUserData} />}</>;
      default:
        return <KoraPayments key={key} setUserData={setUserData} />//<>{getCurrencyCode() === "KES" ? <PaystackPaymentsV1 setUserData={setUserData} /> : <KoraPayments key={key} setUserData={setUserData} />}</>;
    }
